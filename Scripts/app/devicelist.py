...
device_list=Blueprint('device_list',__name__)

# 6. 获取设备的列表
@device_list.route('/getdevicelist',methods=['POST'])
def getdevicelist():
    result = []
    try:
...
# 7. 获取设备的某一状态
@device_list.route('/getdevicestate',methods=['POST'])
def getdevicestate():
    try:
        token = request.json['token']
...
# 8. 获取某一设备历史数据
@device_list.route('/getdevicedata',methods=['POST'])
def getdevicedata():
    result=[]
    try:
        token = request.json['token']
...
# 9.获取某设备详细信息
@device_list.route('/getdevevicedetails',methods=['POST'])
def getdevicedetail():
    try:
        token = request.json['token']
        deviceid = request.json['deviceid']
...
'''
#设备更新进数据库
@device_list.route('/devicelist/add',methods=['POST'])
def add():
    try:
...
#设备更新名称
@device_list.route('/devicelist/changename',methods=['POST'])
def changename():
    try:
        id=request.values.get('id','default')
...
#设备更新所属用户
@device_list.route('/devicelist/updateuserid',methods=['POST'])
def updateid():
    try:
        id=request.values.get('id','default')
