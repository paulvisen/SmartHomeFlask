...
timing_task=Blueprint('timing_task',__name__)

# 10. 设置定时任务
@timing_task.route('/settimingtask',methods=["POST"])
def settimingtask():
    try:
...
@timing_task.route('/timingtask/add',methods=["POST"])
def add():
    try:
...
@timing_task.route('/timingtask/delete',methods=["POST"])
def delete():
    try:
        id=request.values.get('id','default')
        taskcode=request.values.get('taskcode','default')
